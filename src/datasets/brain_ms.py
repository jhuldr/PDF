'''
A longitudinal brain multiple sclerosis dataset.
'''
...
def get_time(path: str) -> float:
    ...
    time = os.path.basename(path).replace('time_', '').split('-')[0].replace('.png', '')
    ...
    def __init__(self,
        ...
        for folder in self.all_patient_folders:
            ...
            for curr_slice in slice_arr:
                paths = sorted(glob('%s/time*.png' % curr_slice))
...
class BrainMSSegDataset(Dataset):
    ...
    def __init__(self,
        ...
        all_patient_folders = sorted(glob('%s/%s/*/' % (base_path, image_folder)))
...
class BrainMSMaskDataset_1(Dataset):
    """
    Multiple sclerosis longitudinal dataset with IMAGE + MASK trajectories.
    ...
    def __init__(self,
        ...
        for patient_folder in tqdm(self.all_patient_folders):
            ...
            for slice_folder in slice_folders:
                img_paths = sorted(glob(f'{slice_folder}/time_*.png'))
                img_paths = [p for p in img_paths]

                msk_paths = [
                    p.replace(image_folder, mask_folder)
                     .replace('.png', '_MS_mask.png')
                    for p in img_paths
                ]
...
class BrainMSMaskDataset(Dataset):
    """
    Multiple sclerosis longitudinal dataset with IMAGE + MASK trajectories.
    Slice-level trajectories, patient-level split.
    """
...
class BrainMSMaskSubset(BrainMSMaskDataset):
    """
    Subset for PDF on brain_MS.
    Returns (images, masks, timestamps).
    """

    def __init__(self,
                #  main_dataset: BrainMSMaskDataset,
                #  subset_indices: List[int],
                 main_dataset: BrainMSMaskDataset = None,
                 subset_indices: List[int] = None,
    ...
    def __getitem__(self, idx):
        ...
        # sample random pair (same as BrainMSSubset one_pair)
        ...
        for ip, mp in zip([img_paths[i], img_paths[j]],
            ...
            # MS tumor core: necrosis + enhancement
            # (consistent with BrainMSSegDataset)
            whole_msk = True #False #
            if whole_msk:
                msk = msk > 0
            else:
...
if __name__ == '__main__':
    print('Full set.')
    dataset = BrainMSDataset(max_slice_per_patient=None)
    dataset.return_statistics()

    print('Subset with max 20 slices per patient.')
    dataset = BrainMSDataset(max_slice_per_patient=20)
    dataset.return_statistics()
